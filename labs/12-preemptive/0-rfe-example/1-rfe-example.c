...
// implemented in <1-rfe-asm.S>: takes a pointer to 
// a 2-entry <uint32_t> array holding the <pc> and <cpsr>
void rfe_asm(uint32_t regs[2]);

// implemented in <1-rfe-asm.S>: loads <sp> and 
// calls <rfe_c_code> (below).
...
void notmain(void) {
    ...
    uint32_t regs[2];
    // 1. set the <pc> entry in the array to point to
    //    <rfe_trampoline_asm>.  this assembly routine 
    //    will load a stack pointer and then call 
    //    <rfe_c_code> (above)
    regs[0] = (uint32_t)rfe_trampoline_asm;
