...
void rpi_exit(int exitcode) {
    ...
    if (Q_empty(&runq)) {
        ...
        while (cur_thread->wakeup_time && *cur_thread->wakeup_time > timer_get_usec()) {
             Q_append(&runq, cur_thread); 
            
             if (Q_empty(&runq)) {
                 while (*cur_thread->wakeup_time > timer_get_usec()) { ; }
                 break; 
             }
             cur_thread = Q_pop(&runq);
        }
...
void rpi_yield(void) {
    ...
    // Clear wakeup time if it expired (just housekeeping)
    if (prev_thread->wakeup_time != NULL && *prev_thread->wakeup_time <= timer_get_usec()) {
        prev_thread->wakeup_time = NULL;
    }

    Q_append(&runq, prev_thread); 

    while (cur_thread->wakeup_time != NULL && *cur_thread->wakeup_time > timer_get_usec()) {
        rpi_thread_t *sleeper = cur_thread;
        Q_append(&runq, sleeper); // Put sleeper at BACK
...
void sleep_until_us(uint32_t us) {
    cur_thread->wakeup_time_val = us + timer_get_usec();
    cur_thread->wakeup_time = &cur_thread->wakeup_time_val;
    rpi_yield();
}
