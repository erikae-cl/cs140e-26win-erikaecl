...
void simple_boot(int fd, uint32_t boot_addr, const uint8_t *buf, unsigned n) { 
    ...
    while((op = get_uint32(fd)) == GET_PROG_INFO) {
        // trace_get32(fd);
    } 
    if(trace_p) {
        trace("GET32:%x [%s]\n", op, boot_op_to_str(op));
    ...
    // unsigned got = trace_get8(fd);

 
    boot_check(fd, "did not receive GET_CODE", GET_CODE, op);
    uint32_t checksum = trace_get32(fd);
    boot_check(fd, "did not receive checksum", checksum, crc32(buf,n));
