...
void uart_init(void) {
    ...
    uint32_t enb_val = GET32(AUX_ENABLES);

    PUT32(AUX_ENABLES, enb_val | 0b1);
    dev_barrier();
...
int uart_get8(void) {
    ...
    unsigned data = GET32(AUX_MU_IO_REG) & 0xFF;
    dev_barrier();
    return data;
    // }
}
...
int uart_can_put8(void) {
    uint32_t tx_mt = GET32(AUX_MU_LSR_REG);
    tx_mt = tx_mt & 0b100000;
    if (tx_mt) {
        return 1;
    }
    return 0;
...
int uart_put8(uint8_t c) {
    dev_barrier();
    uint32_t can_put = uart_can_put8();
    while (!can_put) {
        can_put = uart_can_put8();
    }


    // uint32_t data = GET32(AUX_MU_IO_REG) & ~(0xff);
...
int uart_has_data(void) {
    // TODO: implement this!
    uint32_t lsr_val = GET32(AUX_MU_LSR_REG);
    lsr_val &= 0b1;
    if (lsr_val) {
        return 1;
    }
...
int uart_tx_is_empty(void) {
    dev_barrier();
    uint32_t tx_val = GET32(AUX_MU_STAT_REG);
    tx_val = tx_val & 0b1000000000;
    dev_barrier();
    return tx_val;
